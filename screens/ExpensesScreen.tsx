...
const ExpenseActionsModal: React.FC<{
    expense: Expense;
    onClose: () => void;
    onEdit: () => void;
    onDelete: () => void;
}> = ({ expense, onClose, onEdit, onDelete }) => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-sm flex flex-col dark:bg-slate-800">
            <div className="p-4 border-b dark:border-slate-700">
                <h2 className="text-xl font-bold text-center text-slate-800 dark:text-slate-100">{expense.description}</h2>
            ...
            <div className="p-4 space-y-3">
                <button onClick={onEdit} className="w-full flex items-center gap-3 p-3 rounded-lg bg-slate-100 hover:bg-slate-200 transition-colors dark:bg-slate-700 dark:hover:bg-slate-600">
                    ...
                    <span className="font-semibold text-slate-800 dark:text-slate-100">Editar Gasto</span>
                </button>
                <button onClick={onDelete} className="w-full flex items-center gap-3 p-3 rounded-lg bg-slate-100 hover:bg-slate-200 transition-colors dark:bg-slate-700 dark:hover:bg-slate-600">
                    ...
                    <span className="font-semibold text-slate-800 dark:text-slate-100">Eliminar Gasto</span>
                </button>
            </div>
            <div className="flex justify-end gap-2 p-4 bg-slate-50 border-t rounded-b-lg dark:bg-slate-800/50 dark:border-slate-700">
                <button type="button" onClick={onClose} className="px-4 py-2 bg-slate-200 hover:bg-slate-300 rounded-md transition-colors dark:bg-slate-600 dark:hover:bg-slate-500 dark:text-slate-100">Cancelar</button>
...
const ExpensesScreen: React.FC = () => {
    ...
    const categoryIcons = {
        Varios: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h7" /></svg>,
        Inventario: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" /></svg>,
        Servicios: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
        Alquiler: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>,
    };
        ...
        <div>
            ...
            <div className="p-4 space-y-3">
                {expenses.map(e => (
                    <button
                        key={e.id}
                        onClick={() => setExpenseForActions(e)}
                        className="w-full text-left bg-white p-4 rounded-lg shadow-sm flex justify-between items-center dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors"
                    >
            ...
            {expenseForActions && (
                <ExpenseActionsModal
                    ...
                    onEdit={() => {
                        handleOpenEditModal(expenseForActions);
                        setExpenseForActions(null);
                    }}
                    onDelete={() => {
                        setExpenseToDelete(expenseForActions);
                        setExpenseForActions(null);
                    }}
                />
            )}
